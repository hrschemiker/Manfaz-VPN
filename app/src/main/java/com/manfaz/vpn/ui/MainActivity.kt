...
class MainActivity : ComponentActivity() {
    ...
    ) { /* result is intentionally ignored; the connection proceeds either way */ }
    ...
    override fun onResume() {
        super.onResume()
        refreshVpnState()
        // A dynamic receiver in the isolated core can be between process scheduling slices
        // during Activity resume. One short retry makes foreground state reconciliation
        // deterministic without starting or stopping the VPN service.
    ...
    private fun requestConnect() {
        ...
        if (prepare != null) { vpnConsent.launch(prepare); return }
        requestNotifPermissionIfNeeded()
        vm.toggleConnection()
    }
    ...
    private fun onConnectServer() {
        val prepare = VpnService.prepare(this)
        if (prepare != null) { pendingSpecific = true; vpnConsent.launch(prepare); return }
        requestNotifPermissionIfNeeded()
        vm.connectSelected()
    }

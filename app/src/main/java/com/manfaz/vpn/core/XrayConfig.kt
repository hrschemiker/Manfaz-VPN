...
object XrayConfig {
    ...
    private fun streamSettings(s: ServerConfig, ipv6Mode: Ipv6Mode): JSONObject {
        ...
        val endpointStrategy = when {
            ...
            ipv6Mode == Ipv6Mode.TUNNEL -> "UseIPv4v6"
            else -> "UseIPv4"
        }
        val ss = JSONObject()
            .put("network", net)
        ...
        when (s.security) {
            "tls" -> {
                ss.put("security", "tls")
                val tlsName = s.sni.ifBlank { s.host }
                val tls = JSONObject().put("allowInsecure", false)
                if (tlsName.isNotBlank()) tls.put("serverName", tlsName)
                if (s.alpn.isNotBlank()) tls.put("alpn", JSONArray(s.alpn.split(",")))
                tls.put("fingerprint", s.fingerprint.ifBlank { "chrome" })
